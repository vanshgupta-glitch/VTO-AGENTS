...
interface RuntimesFile {
  [key: string]: RuntimeConfig;
}
...
program
  ...
  .action(async () => {
    ...
    for (const c of configs) {
      ...
      if (existsSync(p)) console.log(`✅ ${c}`);
      else { console.log(`❌ ${c} missing`); allOk = false; }
    }

    // 2. Check .secrets.env
    const secretsPath = join(CONFIG_DIR, '.secrets.env');
    if (existsSync(secretsPath)) console.log('✅ .secrets.env');
    else { console.log('❌ .secrets.env missing (copy from .secrets.env.example)'); allOk = false; }
    ...
    for (const a of agents) {
      ...
      if (existsSync(yamlPath) && existsSync(sysPath)) {
        ...
        console.log(`❌ agents/${a} missing files`); allOk = false;
      }
    }

    // 4. Check CLI versions
    const runtimes = yaml.load(readFileSync(join(CONFIG_DIR, 'runtimes.yaml'), 'utf8')) as RuntimesFile;
    ...
    for (const t of requiredTokens) {
      if (process.env[t]) console.log(`✅ ${t}`);
      else console.log(`❌ ${t} not set`);
    }
...
program
  ...
  .action((id, opts) => {
    ...
    const displayName = id.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
...
program
  ...
  .action(() => {
    ...
    const agents = readdirSync(AGENTS_DIR).filter(d => d !== '_template');
    for (const a of agents) {
      const agentYaml = yaml.load(readFileSync(join(AGENTS_DIR, a, 'agent.yaml'), 'utf8')) as AgentYaml;
