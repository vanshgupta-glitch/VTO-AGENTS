...
export async function bootstrapIds() {
    ...
    if (!token) {
        console.error('SLACK_BOT_ADMIN not set');
        process.exit(1);
    }
...
export async function bootstrapGenerateBridgeConfig() {
    ...
    for (const a of agents) {
        const agentYaml = yaml.load(readFileSync(join(agentsDir, a, 'agent.yaml'), 'utf8'));
        if (!agentYaml.enabled)
            continue;
        const agentConfig = {
    ...
    const swarmConfig = yaml.load(readFileSync(join(CONFIG_DIR, 'swarm.config.yaml'), 'utf8'));
