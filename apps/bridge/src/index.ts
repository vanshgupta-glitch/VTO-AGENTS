...
import type { SlackPort } from './slack.js';
...
function preflight(): void {
  if (DRY) return;
  const missing = (['SLACK_APP_TOKEN', 'SLACK_BOT_ADMIN'] as const).filter(
    (k) => !(config.secrets.get(k) ?? process.env[k]),
  );
...
const channelName = (idOrName: string): string => CHANNEL_NAMES.get(idOrName) ?? idOrName;

function replyText(r: DispatchResult): string {
  const stage = r.ok ? 'complete' : 'failed';
  return `${formatHeader(r.taskId, 0, stage)}\n${r.reply || r.error || '(no output)'}`;
}

async function reportFailure(r: DispatchResult): Promise<void> {
  if (r.ok) return;
  await slack.post({
    agent: 'admin',
 ...
 */
async function handleCli(req: DispatchRequest): Promise<DispatchResult> {
