...
interface RuntimeEntry {
  ...
  model_override_template?: string[];
}
...
export function createRunner(root: string, timeoutSeconds: number): Runner {
  ...
  function tooLong(entry: RuntimeEntry, prompt: string): RunResult | null {
    if (!entry.inline_prompt || prompt.length <= MAX_INLINE_PROMPT) return null;
    return fail(
      `prompt is ${prompt.length} chars but this runtime passes it on the command ` +
  ...
  return {
    async run(agent, prompt) {
      ...
      if (!entry?.bin) return fail(`no runtime '${agent.runtime}' with a bin in runtimes.yaml`);
