 ...
 * listener.
 */
export function createSlack(config) {
    ...
    function clientFor(agent) {
        ...
        if (!token)
            throw new Error(`no Slack token for '${agent}' (looked for ${envName})`);
    ...
    return {
        ...
        async start(onMention) {
            ...
            if (!appToken)
                throw new Error('SLACK_APP_TOKEN missing - the listener cannot start');
            ...
            const handle = async (args) => {
                ...
                if (selfUserId && sender === selfUserId)
                    return;
                ...
                if (!target)
                    return; // not addressed to any agent
                if (target === sender)
                    return; // an agent mentioning itself
                try {
                    await onMention({
                        agent: target,
