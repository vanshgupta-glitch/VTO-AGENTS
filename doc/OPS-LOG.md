...
- **T042 GPU blocker SOLVED via Kaggle** (operator directive): `rkumar-vto/tools/model-training/` — self-contained Kaggle GPU script (BiSeNet n_classes=3, AMP, resume, ONNX export + .wasm rename, gate report) + pytest gates + kernel-metadata. Datasets: Kaggle `mantasu/glasses-segmentation-synthetic-dataset` (Lyu-style FRAME masks → exact 3-class labels), `ipythonx/celebamaskhq` (eye_g real-domain check), optional MS FaceSynthetics + mantasu/glasses-detector (MIT teacher). T042 issue doc updated — human step: run the notebook, drop passing `out/` into face-parsing.PyTorch\res\.
- **COMMITTED + PUSHED** (operator-directed): vault `54d0647` → pgmq-dual-gateway AND main (Vansh pulls main); product repo `a87ba32` → card-face-width (model-training, video harness code, issue docs, vto-phase1 toml; .gitignore now blocks test videos/logs/.swarm-tasks — card-face-width never had those rules and 3.7 GB of clips nearly staged). Excluded from commits: other-project vault dirs (Agentic OS, I Love Gurgaon), scratch files, machine-local/secret configs.
- **doc/VANSH-CUTOVER.md** — step-by-step rejoin runbook (kill-all-old → pull main → build → hermes+ops roles first, coder/openclaw only after repointing his live-repo workspace landmine → daemon only, NO second gateway without a joint D-036 flip → verify via swarm-audit). `tools/start-swarm.ps1` now sets SWARM_CLAIM_VT=2400.
